...
class BL {
	...
	function __construct(){
		$ekse = $this->postEksekusi($this->getResources(), intval($_GET['jumlah']), intval($_GET['offset']));
	}
	...
	private function postEksekusi($data, $num, $offset = 0){
		if(is_array($data)&&!empty($data)){
			...
			$fp = fopen(__DIR__.'/VOC_LIVE.txt', 'a');
			for($i=0;$i<$num;$i++){
				...
				if($x[2]==200){
					...
					print "[$nums] $voc =|> [ ".$w->message." ]<br>";
				}else
					print "[$nums] $voc =|> [ INTERNAL SERVER ERROR - $x[3] ]<br>";
				ob_flush();
        		flush();
			}
			fclose($fp);
